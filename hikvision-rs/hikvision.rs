...
async fn main() {
    ...
    for received in rx {
        ...
        println!("{}", received.clone());
    }
}

fn on_vmd(date: String, recipients: &Vec<Value>, auth_token: String) {
    let timestamp = DateTime::parse_from_str(&date, "%+").unwrap();
    let date = chrono::offset::Local
        .timestamp_opt((timestamp.timestamp()).try_into().unwrap(), 0)
        .unwrap();
    let url = format!(
        "{}/{}.jpg",
    ...
    download_snapshot(timestamp.timestamp());
